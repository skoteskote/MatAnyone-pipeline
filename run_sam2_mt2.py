"""SAM 2 multi-object on mask-test-2 (downsampled to 960)."""
import os
...
PROMPTS = {
    1: ("person",     [0, 0, 530, 960]),
    2: ("blue_box",   [430, 175, 700, 470]),
    3: ("wooden_box", [50, 380, 470, 680]),
    4: ("hand_right", [380, 380, 600, 530]),
    5: ("hand_left",  [0, 540, 220, 720]),
    6: ("desk",       [0, 690, 960, 960]),
}
...
imageio.mimwrite(f"{OUT_DIR}/mask-test-2_{SUFFIX}_fgr.mp4", np.stack(fgrs), fps=FPS, quality=7)
imageio.mimwrite(f"{OUT_DIR}/mask-test-2_{SUFFIX}_pha.mp4", np.stack(phas), fps=FPS, quality=7)
print(f"wrote {OUT_DIR}/mask-test-2_{SUFFIX}_(fgr|pha).mp4")
